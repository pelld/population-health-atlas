...
// ============================================================
const realData = window.ATLAS_DATA || null;
const mapAreas = window.ICB_BOUNDARIES || regions;
const conditions = realData?.conditions || ["Diabetes", "Depression", "Hypertension", "COPD", "CHD", "Atrial fibrillation", "CKD", "Asthma"];
...
const pairs = realData ? realData.pairs.map(pair => [pair.first, pair.second, pair.correlation]) : [["Diabetes","Hypertension",0.74],["CHD","Atrial fibrillation",0.67],["COPD","Depression",0.61],["Diabetes","CHD",0.56],["Hypertension","CKD",0.51]];
const firstCondition = conditions.find(condition => condition.toLowerCase().includes("diabetes")) || conditions[0];
let state = { condition:firstCondition, pair:"None", mode:"recorded", selected:mapAreas.find(area => realData?.areas[area.id])?.id || mapAreas[0].id };
...
el("dataStatus").textContent = realData ? "Official data" : "Demonstration version";
el("dataStatusText").textContent = realData ? `QOF ${realData.year}, adjusted using the English Indices of Deprivation ${realData.deprivationYear}.` : "The figures illustrate the proposed analysis and are not published QOF results.";
el("geographyLabel").textContent = realData ? realData.geography : "Illustrative NHS regions";
el("methodStatus").textContent = realData ? `The displayed prevalence is from the NHS QOF ${realData.year} publication. Deprivation adjustment uses the English Indices of Deprivation ${realData.deprivationYear}.` : "The site is currently using fabricated regional values while the official-data pipeline is being prepared.";

// ============================================================
// 02. MAP CALCULATIONS AND RENDERING
// ============================================================
function values() {
  if (realData) return mapAreas.filter(area => realData.areas[area.id]?.[state.mode === "recorded" ? "prevalence" : "residual"]?.[state.condition] != null).map(area => ({ ...area, value:realData.areas[area.id][state.mode === "recorded" ? "prevalence" : "residual"][state.condition] }));
  return mapAreas.map(area => ({ ...area, value:state.mode === "recorded" ? area.base * factors[state.condition] : (area.base * factors[state.condition] - 8.8 * factors[state.condition]) - area.imd }));
}
...
el("areaSearch").addEventListener("input", event => {
  const query = event.target.value.toLowerCase(), matches = query ? mapAreas.filter(area => area.name.toLowerCase().includes(query) && (!realData || realData.areas[area.id])) : [];
